...
void detectHipLift(Map<PoseLandmarkType, PoseLandmark> landmarks) {
  ...
  // Calculate hip height (Y-position)
  ...
  // Check if the knees are bent (indicating a potential hip lift position)
  bool areKneesBent = leftKneeHeight > leftHipHeight && rightKneeHeight > rightHipHeight;

  // Track upward movement of the hips (Y-position change)
  bool isLifting = leftHipHeight < leftKneeHeight && rightHipHeight < rightKneeHeight; // Hips higher than knees

  if (isLifting && areKneesBent) {
    if (!isLiftingHips) {
      isLiftingHips = true;
    }
  } else if (!isLifting && isLiftingHips) {
    hipLiftCount++;
    isLiftingHips = false;
