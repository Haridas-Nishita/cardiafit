...
void detectHipSwingLeft(Map<PoseLandmarkType, PoseLandmark> landmarks) {
  ...
  // Detect lateral movement (to the left)
  bool isMovingLeft = leftHipX < rightHipX; // Left hip moves to the left relative to right hip

  // Detect if the person is performing a swing motion (knee/ankle on the left leg should be aligned)
  bool isLeftLegAligned = leftKneeX == leftAnkle.x;

  // Check for hip swing left motion
  if (isMovingLeft && isLeftLegAligned) {
    if (!isSwingingLeft) {
      isSwingingLeft = true;
    }
  } else if (!isMovingLeft && isSwingingLeft) {
    hipSwingLeftCount++;
