...
int rightLegKickbackCount = 0;
bool isKickingBackRight = false;

void detectRightLegKickback(Map<PoseLandmarkType, PoseLandmark> landmarks) {
  ...
  // Calculate the Y-position of the hips and knees
  double rightHipX = rightHip.x;
  double rightHipY = rightHip.y;
  double leftHipX = leftHip.x;
  double leftHipY = leftHip.y;

  double rightKneeX = rightKnee.x;
  double rightKneeY = rightKnee.y;
  double rightAnkleX = rightAnkle.x;
  double rightAnkleY = rightAnkle.y;

  // Check if the right leg is extending backward
  bool isLegExtendingBack = rightKneeY < rightHipY && rightAnkleY < rightHipY; // Right leg moving backward

  // Ensure the knee is straight during the kickback (knee alignment)
  bool isKneeStraight = rightKneeY == rightAnkleY;

  // Check for the kickback movement
  if (isLegExtendingBack && isKneeStraight) {
    if (!isKickingBackRight) {
      isKickingBackRight = true;
    }
  } else if (!isLegExtendingBack && isKickingBackRight) {
    rightLegKickbackCount++;
    isKickingBackRight = false;
