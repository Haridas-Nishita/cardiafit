...
void detectCalfRaise(Map<PoseLandmarkType, PoseLandmark> landmarks) {
  ...
  // Calculate ankle height (Y-position)
  double leftAnkleHeight = leftAnkle.y;
  double rightAnkleHeight = rightAnkle.y;

  // Calculate knee and hip positions to ensure minimal knee movement
  double leftKneeHeight = leftKnee.y;
  double rightKneeHeight = rightKnee.y;
  double leftHipHeight = leftHip.y;
  double rightHipHeight = rightHip.y;

  // Check for lifting (heels coming off the ground)
  bool isLifting = leftAnkleHeight < leftKneeHeight && rightAnkleHeight < rightKneeHeight;
  
  // Check if the person is maintaining the calf raise (heels raised)
  bool isAtTopPosition = isLifting && (leftAnkleHeight < leftHipHeight) && (rightAnkleHeight < rightHipHeight);

  if (isAtTopPosition) {
    if (!isRaising) {
      isRaising = true;
    }
  } else if (!isAtTopPosition && isRaising) {
    calfRaiseCount++;
    isRaising = false;
