...
void detectShoulderShrug(Map<PoseLandmarkType, PoseLandmark> landmarks) {
  ...
  // Get Y-coordinate values for the shoulders
  double leftShoulderY = leftShoulder.y;
  double rightShoulderY = rightShoulder.y;

  // Define threshold values for detecting shoulder shrug movement
  // These values can be adjusted based on testing or user movement
  double maxShoulderHeight = 0.2;  // Maximum height for the shoulder during shrug
  double minShoulderHeight = 0.3;  // Minimum height for shoulder when relaxed

  // Detect when shoulders are moving up (shrugging up)
  bool isShruggingUpLeft = leftShoulderY < maxShoulderHeight; 
  bool isShruggingUpRight = rightShoulderY < maxShoulderHeight;

  // Detect when shoulders are relaxed (coming down from a shrug)
  bool isShruggingDownLeft = leftShoulderY > minShoulderHeight;
  bool isShruggingDownRight = rightShoulderY > minShoulderHeight;

  // Check for shrug up motion
  if (isShruggingUpLeft && isShruggingUpRight && !isShruggingUp) {
    isShruggingUp = true;
  }
  // Check for shrug down motion
  else if (isShruggingDownLeft && isShruggingDownRight && isShruggingUp) {
    shoulderShrugCount++;
    isShruggingUp = false;
