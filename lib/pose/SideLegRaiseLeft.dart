...
class _MyHomePageState extends State<MyHomePage> {
  ...
  doPoseEstimationOnFrame() async {
    ...
    if(inputImage != null){
      ...
      _scanResults = poses;
      if(poses.length>0) {
        detectSideLegRaiseLeftLeg(poses.first.landmarks);
      }
    }
    setState(() {
      _scanResults;
  ...
  int leftSideLegRaiseCount = 0;
bool isLeftLegRaising = false;

void detectSideLegRaiseLeftLeg(Map<PoseLandmarkType, PoseLandmark> landmarks) {
  final leftHip = landmarks[PoseLandmarkType.leftHip];
  final leftKnee = landmarks[PoseLandmarkType.leftKnee];
  final leftAnkle = landmarks[PoseLandmarkType.leftAnkle];
  final rightHip = landmarks[PoseLandmarkType.rightHip]; // For stability reference

  if (leftHip == null || leftKnee == null || leftAnkle == null || rightHip == null) {
    return; // Skip detection if any key landmark is missing
  }

  // Calculate the angle of the left leg relative to the hip
  double legRaiseAngle = calculateAngle(leftHip, leftKnee, leftAnkle);

  // Threshold for detecting a raise
  bool isLegRaised = legRaiseAngle > 45;  // Adjust based on testing

  if (isLegRaised && !isLeftLegRaising) {
    isLeftLegRaising = true;
  } else if (!isLegRaised && isLeftLegRaising) {
    leftSideLegRaiseCount++;  // Count a complete rep when the leg returns down
    isLeftLegRaising = false;
    setState(() {});  // Update UI
  }
}
