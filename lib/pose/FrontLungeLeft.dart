...
void detectFrontLungeLeft(Map<PoseLandmarkType, PoseLandmark> landmarks) {
  ...
  // Check for deep lunge (left knee bent and right leg extended)
  bool isDeepLunge = leftKneeAngle < 90 && rightKneeY > leftKneeY;

  // Check for left leg forward motion
  bool isLeftLegForward = leftAnkleY < leftHipY && rightAnkleY > rightHipY; // Left leg is forward and right leg is extended

  // Detect when the person is performing a left front lunge
  if (isLeftLegForward && isDeepLunge) {
    if (!isLungingLeft) {
      isLungingLeft = true;
    }
  } else if (!isDeepLunge && isLungingLeft) {
    leftLungeCount++;
    isLungingLeft = false;
