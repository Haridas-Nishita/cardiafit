...
int leftLegKickbackCount = 0;
bool isKickingBackLeft = false;

void detectLeftLegKickback(Map<PoseLandmarkType, PoseLandmark> landmarks) {
  ...
  double leftKneeX = leftKnee.x;
  double leftKneeY = leftKnee.y;
  double leftAnkleX = leftAnkle.x;
  double leftAnkleY = leftAnkle.y;

  // Check if the left leg is extending backward
  bool isLegExtendingBack = leftKneeY < leftHipY && leftAnkleY < leftHipY; // Left leg moving backward

  // Ensure the knee is straight during the kickback (knee alignment)
  bool isKneeStraight = leftKneeY == leftAnkleY;

  // Check for the kickback movement
  if (isLegExtendingBack && isKneeStraight) {
    if (!isKickingBackLeft) {
      isKickingBackLeft = true;
    }
  } else if (!isLegExtendingBack && isKickingBackLeft) {
    leftLegKickbackCount++;
    isKickingBackLeft = false;
