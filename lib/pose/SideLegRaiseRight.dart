...
class _MyHomePageState extends State<MyHomePage> {
  ...
  doPoseEstimationOnFrame() async {
    ...
    if(inputImage != null){
      ...
      _scanResults = poses;
      if(poses.length>0) {
        detectSideLegRaiseRightLeg(poses.first.landmarks);
      }
    }
    setState(() {
      _scanResults;
...
bool isRaisingLeg = false;

void detectSideLegRaiseRightLeg(Map<PoseLandmarkType, PoseLandmark> landmarks) {
  final rightHip = landmarks[PoseLandmarkType.rightHip];
  final rightKnee = landmarks[PoseLandmarkType.rightKnee];
  final rightAnkle = landmarks[PoseLandmarkType.rightAnkle];
  final leftHip = landmarks[PoseLandmarkType.leftHip];  // For stability reference

  if (rightHip == null || rightKnee == null || rightAnkle == null || leftHip == null) {
    return; // Skip detection if any key landmark is missing
  }

  // Calculate the angle of the right leg relative to the hip
  ...
  if (isLegRaised && !isRaisingLeg) {
    isRaisingLeg = true;
  } else if (!isLegRaised && isRaisingLeg) {
    sideLegRaiseCount++;  // Count a complete rep when the leg returns down
    isRaisingLeg = false;
    setState(() {});  // Update UI
  }
}
