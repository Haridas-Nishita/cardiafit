...
int punchOutCount = 0;
bool isPunchingOutRight = false;

void detectRightPunchOut(Map<PoseLandmarkType, PoseLandmark> landmarks) {
  ...
  // Calculate the distance moved by the wrist in the x-direction (forward punch)
  double rightShoulderX = rightShoulder.x;
  double rightShoulderY = rightShoulder.y;
  double rightElbowX = rightElbow.x;
  double rightElbowY = rightElbow.y;
  double rightWristX = rightWrist.x;
  double rightWristY = rightWrist.y;

  // Detect if the right arm is fully extended in front of the body
  bool isArmFullyExtended = rightElbowX > rightShoulderX && rightWristX > rightShoulderX && rightWristY > rightShoulderY;

  // Check if the arm is at the starting bent position (before punching)
  bool isArmBent = rightElbowX < rightShoulderX && rightWristX < rightElbowX;

  // Detect punch out
  if (isArmFullyExtended && !isPunchingOutRight) {
    isPunchingOutRight = true;
  }
  // Detect when the arm returns to the starting position
  else if (isArmBent && isPunchingOutRight) {
    punchOutCount++;
    isPunchingOutRight = false;
