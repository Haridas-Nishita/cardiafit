...
void detectFrontLungeRight(Map<PoseLandmarkType, PoseLandmark> landmarks) {
  ...
  // Check for deep lunge (right knee bent and left leg extended)
  bool isDeepLunge = rightKneeAngle < 90 && leftKneeY > rightKneeY;

  // Check for right leg forward motion
  bool isRightLegForward = rightAnkleY < rightHipY && leftAnkleY > leftHipY; // Right leg is forward and left leg is extended

  // Detect when the person is performing a right front lunge
  if (isRightLegForward && isDeepLunge) {
    if (!isLungingRight) {
      isLungingRight = true;
    }
  } else if (!isDeepLunge && isLungingRight) {
    rightLungeCount++;
