...
int hipSwingRightCount = 0;
bool isSwingingRight = false;

void detectHipSwingRight(Map<PoseLandmarkType, PoseLandmark> landmarks) {
  ...
  // Calculate hip position (Y-coordinate) and movement (X-coordinate)
  ...
  // Detect lateral movement (to the right)
  bool isMovingRight = rightHipX > leftHipX; // Right hip moves to the right relative to left hip

  // Detect if the person is performing a swing motion (knee/ankle on the right leg should be aligned)
  bool isRightLegAligned = rightKneeX == rightAnkle.x;

  // Check for hip swing right motion
  if (isMovingRight && isRightLegAligned) {
    if (!isSwingingRight) {
      isSwingingRight = true;
    }
  } else if (!isMovingRight && isSwingingRight) {
    hipSwingRightCount++;
