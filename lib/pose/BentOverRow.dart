...
void detectBentOverRow(Map<PoseLandmarkType, PoseLandmark> landmarks) {
  ...
  // Check for the body position: lean forward (not too deep)
  bool isLeaningForward = shoulderY < hipY; // Shoulders lower than hips (lean forward)
  bool isPulling = avgElbowAngle < 90; // Elbows bent when pulling

  // Bent-over row logic
  if (isLeaningForward && isPulling) {
    if (!isRowing) {
      isRowing = true;
    }
  } else if (!isPulling && isRowing) {
    rowCount++;
    isRowing = false;
